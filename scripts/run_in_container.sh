...
# Remove any existing container named 'neopilot'.
docker rm -f neopilot || true

# Run the container with the specified directory mounted at the same path inside the container.
docker run --name neopilot -d \
  -e OPENAI_API_KEY \
  --cap-add=NET_ADMIN \
  --cap-add=NET_RAW \
  -v "$WORK_DIR:$WORK_DIR" \
...
docker exec neopilot bash -c "sudo /usr/local/bin/init_firewall.sh"

# Execute the provided command in the container, ensuring it runs in the work directory.
# We use a parameterized bash command to safely handle the command and directory.
docker exec neopilot bash -c "cd \"$WORK_DIR\" && neopilot --dangerously-auto-approve-everything -q \"$@\""

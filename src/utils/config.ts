...
export const DEFAULT_AGENTIC_MODEL: Model = {
  provider: "openai",
  model: "o4-mini",
};
export const DEFAULT_FULL_CONTEXT_MODEL: Model = {
  provider: "openai",
  model: "gpt-4.1",
};
...
export const CONFIG_DIR = join(homedir(), ".neopilot");
...
// ---------------------------------------------------------------------------
// Project doc support (neopilot.md)
// ---------------------------------------------------------------------------

export const PROJECT_DOC_MAX_BYTES = 32 * 1024; // 32 kB

const PROJECT_DOC_FILENAMES = ["neopilot.md", ".neopilot.md", "NEOPILOT.md"];
...
/**
 * Load the project documentation markdown (neopilot.md) if present. If the file
...
export function loadProjectDoc(cwd: string, explicitPath?: string): string {
  ...
  if (explicitPath) {
    ...
    if (!existsSync(filepath)) {
      ...
      console.warn(`neopilot: project doc not found at ${filepath}`);
      filepath = null;
    }
  } else {
  ...
  try {
    ...
    if (buf.byteLength > PROJECT_DOC_MAX_BYTES) {
      // eslint-disable-next-line no-console
      console.warn(
        `neopilot: project doc '${filepath}' exceeds ${PROJECT_DOC_MAX_BYTES} bytes – truncating.`
      );
    }
...
export const loadConfig = (
  ...
  const shouldLoadProjectDoc =
    ...
    process.env["NEOPILOT_DISABLE_PROJECT_DOC"] !== "1";
  ...
  if (shouldLoadProjectDoc) {
    ...
    if (projectDocPath) {
      if (isLoggingEnabled()) {
        log(
          `[neopilot] Loaded project doc from ${projectDocPath} (${projectDoc.length} bytes)`
        );
      }
    } else {
      if (isLoggingEnabled()) {
        log(`[neopilot] No project doc found in ${cwd}`);
      }
    }
  }
  ...
  const defaultModel = options.isFullContext
    ? DEFAULT_FULL_CONTEXT_MODEL
    : DEFAULT_AGENTIC_MODEL;

  const config: AppConfig = {
    provider: storedProvider ?? defaultModel.provider,
    model: storedModel ?? defaultModel.model,
  ...
  // materialised ~/.neopilot/config.json file on first execution.  This mirrors
  ...
  try {
    if (!existsSync(actualConfigPath)) {
      ...
      // pinning users to an old default after upgrading Neopilot.
      const ext = extname(actualConfigPath).toLowerCase();
      if (ext === ".yaml" || ext === ".yml") {
    ...
    // block the CLI from starting.  A future explicit `neopilot config` command

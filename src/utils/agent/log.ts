...
/**
 * Creates a .log file for this session, but also symlinks neopilot-cli-latest.log
 * to the current log file so you can reliably run:
 *
 * - Mac/Windows: `tail -F "$TMPDIR/neopilot-cli/neopilot-cli-latest.log"`
 * - Linux: `tail -F ~/.local/neopilot-cli/neopilot-cli-latest.log`
 */
export function initLogger(): Logger {
  ...
  // it there. On Linux, use ~/.local/neopilot-cli so logs are not world-readable.
  const logDir =
    isMac || isWin
      ? path.join(os.tmpdir(), "neopilot-cli")
      : path.join(os.homedir(), ".local", "neopilot-cli");
  fsSync.mkdirSync(logDir, { recursive: true });
  const logFile = path.join(logDir, `neopilot-cli-${now()}.log`);
  // Write the empty string so the file exists and can be tail'd.
  fsSync.writeFileSync(logFile, "");

  // Symlink to neopilot-cli-latest.log on UNIX because Windows is funny about
  // symlinks.
  if (!isWin) {
    const latestLink = path.join(logDir, "neopilot-cli-latest.log");
    try {

...
// Call this early so `tail -F "$TMPDIR/neopilot-cli/neopilot-cli-latest.log"` works
...
const cli = meow(
  `
  Usage
    $ neopilot [options] <prompt>

  Options
    -h, --help                 Show usage and exit
    -m, --model <model>        Model to use for completions
    -p, --provider <provider>  AI provider to use (default: openai) Values: openai, anthropic, google
    ...
    --no-project-doc           Do not automatically include the repository's 'neopilot.md'
  ...
  Examples
    $ neopilot "Write and run a python program that prints ASCII art"
    $ neopilot -q "fix build issues"
`,
  {
    ...
    flags: {
      ...
      approvalMode: {
        ...
        aliases: ["a"],
        description:
          "Determine the approval mode for Neopilot (default: suggest) Values: suggest, auto-edit, full-auto",
      },
      noProjectDoc: {
        type: "boolean",
        description: "Disable automatic inclusion of project‑level neopilot.md",
      },
      projectDoc: {
        type: "string",
...
const provider = cli.flags.provider as AIProvider;
...
if (!(await isModelSupportedForResponses(config))) {
  // eslint-disable-next-line no-console
  console.error(
    `The model "${config.model}" from provider "${config.provider}" does not appear in the list of models ` +
      `available to your account. Double‑check the spelling or choose another model with the --model and --provider flags.`
  );
  process.exit(1);
}
...
if (quietMode) {
  process.env["NEOPILOT_QUIET_MODE"] = "1";
  if (!prompt || prompt.trim() === "") {
    // eslint-disable-next-line no-console
    console.error(
      'Quiet mode requires a prompt string, e.g.,: neopilot -q "Fix bug #123 in the foobar project"'
    );
    process.exit(1);
  }
  await runQuietMode({
